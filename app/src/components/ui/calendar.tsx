...
function Calendar({
  ...
  transactionDayMap?: Record<string, { hasIncome: boolean; hasExpense: boolean }>
}) {
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        selected:
          "bg-primary text-primary-foreground focus:bg-primary focus:text-primary-foreground hover:bg-primary hover:text-primary-foreground",
        outside: cn(
...
function CalendarDayButton({
  ...
  const dayData = transactionDayMap?.[dateStr];
  ...
  if (dayData) {
    if (dayData.hasIncome && dayData.hasExpense) {
      customStyle = {
        background: 'linear-gradient(135deg, #10b981 50%, #f43f5e 50%)',
        color: '#ffffff'
      };
      customClassName = 'text-white hover:!opacity-95';
    } else if (dayData.hasIncome) {
      customStyle = {
        backgroundColor: '#10b981',
        color: '#ffffff'
      };
      customClassName = 'text-white hover:!bg-[#10b981]/90 hover:!text-white';
    } else if (dayData.hasExpense) {
      customStyle = {
        backgroundColor: '#f43f5e',
        color: '#ffffff'
      };
      customClassName = 'text-white hover:!bg-[#f43f5e]/90 hover:!text-white';
    }
  }

  let cleanedClassName = className;
  if (dayData && modifiers.selected) {
    cleanedClassName = className
      ?.replace(/\bbg-primary\b/g, '')
      ?.replace(/\btext-primary-foreground\b/g, '')
      ?.replace(/\brdp-selected\b/g, '')
      ?.replace(/\brdp-day_selected\b/g, '');
  }
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        modifiers.selected && dayData ? "ring-2 ring-blue-600 dark:ring-blue-400 ring-offset-2 dark:ring-offset-slate-900 font-bold z-10" : "",
        customClassName,
        defaultClassNames.day,
        cleanedClassName
      )}
